...
void Setup(){
  ...
  ////
  if(a0*0.5<1.0){
    printf("THIS PROGRAMA DOES'NT WORK WITH a0<sigma\n");;
    exit(-1);;
  }
  ...
  AreaObs=fpi*diamObs*diamObs*0.25+fpi*diamobsr*diamobsr*0.25+0.5*fpi*diamobsr*diamobsr*0.25;
  Space=L*L-AreaObs;
  Mol=ceil(Density*Space*4*(1./fpi));
  printf("Mol  %d\n",Mol);;
  ///set arrays
  printf("Allocarray!\n");
  Allocarray();
...
void Allocarray(){
  ...
  AllocMem(distr_vy,NPROBV,proba_v);
...
//
 ObsS[4].diam=diamObs;//central one
 //
  ///null obstalces
   ...
   if(Nowcycles==NstepT){
    ...
    //*************************************************
	 ...
	 f_r[pp].N=1.;
       ...
       //**************
       auxT=1.;
       fprintf(fp_prob,"%lf\t%lf\t%lf\t%lf\n",auxT,f_r[pp].Vt,f_r[pp].Vt,f_r[pp].Vn);
       //********************
